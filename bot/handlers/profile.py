...
from bot.database.user_api_methods_for_db import remove_user_settings, get_user_openai_key, get_user_max_tokens
...
async def profile_command(message: types.Message, session_maker: sessionmaker) -> types.Message:
    if await check_subscription(user_id=message.chat.id, session_maker=session_maker):
        ...
        api_key = await get_user_openai_key(user_id=message.from_user.id, session_maker=session_maker)
        max_tokens = await get_user_max_tokens(user_id=message.from_user.id, session_maker=session_maker)
        return await message.answer(f'Профиль\n\nid: {message.chat.id}\nВаш токен: {api_key}\nМаксимальное '
                                    f'количество токенов для ответа: {max_tokens}\n\nДля изменения или удаления настроек'
                                    f' вы можете воспользоваться клавиатурой внизу', reply_markup=builder.as_markup())
...
async def remove_token(callback: types.CallbackQuery, session_maker: sessionmaker) -> types.Message:
    await remove_user_settings(user_id=callback.from_user.id, session_maker=session_maker)
    await callback.message.answer('Ваш токен удален!\nВы по прежнему можете пользоваться бесплатными запросами '
                                         'или купить подписку на месяц')
    await callback.answer()
